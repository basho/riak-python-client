...
class HttpTransport(Transport, HttpConnection, HttpResources, HttpCodec):
    """
    ...
    def delete(self, robj, rw=None, r=None, w=None, dw=None, pr=None, pw=None,
        ...
        url = self.object_path(robj.bucket.name, robj.key,
                               bucket_type=bucket_type, **params)
        use_vclocks = (
            self.tombstone_vclocks() and hasattr(robj, "vclock") and robj.vclock is not None
        )
        if use_vclocks:
    ...
    def stream_buckets(self, bucket_type=None, timeout=None):
        ...
        if not self.bucket_stream():
            raise NotImplementedError(
                f"Streaming list-buckets is not supported on {self.server_version.vstring}",
            )
        bucket_type = self._get_bucket_type(bucket_type)
        url = self.bucket_list_path(bucket_type=bucket_type, buckets="stream", timeout=timeout)
        status, headers, response = self._request("GET", url, stream=True)
    ...
    def clear_bucket_props(self, bucket):
        ...
        if status == 204:
            ...
            raise RiakError(f"Error {status} clearing bucket properties.")
    ...
    def mapred(self, inputs, query, timeout=None):
        ...
        if status != 200:
            raise RiakError(
                f"Error running MapReduce operation. Headers: {repr(headers)} Body: {repr(body)}",
            )
    ...
    def stream_mapred(self, inputs, query, timeout=None):
        ...
        status, headers, response = self._request("POST", url, reqheaders, content, stream=True)
        ...
        if status == 200:
            ...
            raise RiakError(
                ...
                f"Headers: {repr(headers)} Body: {response.read()}",
            )

    def get_index(
        self, bucket, index, startkey, endkey=None, return_terms=None, max_results=None,
        ...
        if term_regex and not self.index_term_regex():
            raise NotImplementedError(
                f"Secondary index term_regex is not supported on {self.server_version.vstring}",
            )
    ...
    def stream_index(self, bucket, index, startkey, endkey=None,
        ...
        if not self.stream_indexes():
            raise NotImplementedError(
                f"Secondary index streaming is not supported on {self.server_version.vstring}",
            )
        ...
        bucket_type = self._get_bucket_type(bucket.bucket_type)
        url = self.index_path(
            bucket.name, index, startkey, endkey, bucket_type=bucket_type, **params,
        )
        status, headers, response = self._request("GET", url, stream=True)
