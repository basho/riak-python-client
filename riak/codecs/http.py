...
class HttpCodec(object):
    ...
    def _parse_body(self, robj, response, expected_statuses):
        ...
        if not status:
            raise RiakError(
                f"Could not contact Riak Server: http://{self._node.host}:{self._node.http_port}!",
            )
        ...
        # If 300(Siblings), apply the siblings to the object
            ...
            if ctype == "multipart/mixed":
                ...
                robj.siblings = [
                    self._parse_sibling(
                        RiakContent(robj), list(part.items()), part.get_payload(),
                    )
                    for part in parts
                ]
                ...
                raise Exception(f"unexpected sibling response format: {ctype}")
    ...
    def _build_put_headers(self, robj, if_none_match=False):
        ...
        if robj.charset is not None:
            content_type = (f'{robj.content_type}; charset="{robj.charset}"')
        else:
        ...
        for key in robj.usermeta.keys():
            headers[f"X-Riak-Meta-{key}"] = robj.usermeta[key]

        for field, value in robj.indexes:
            key = f"X-Riak-Index-{field}"
            if key in headers:
