...
class TwoITests(IntegrationTestBase, unittest.TestCase):
    ...
    def test_secondary_index_store(self):
        ...
        self.assertEqual(["val1a", "val1b"],
                         sorted([y for (x, y) in obj.indexes if x == "field1_bin"]))
        self.assertEqual([1011, 1012],
                         sorted([y for (x, y) in obj.indexes if x == "field1_int"]))
    ...
    def _create_index_objects(self, int_sign=1):
        ...
        o1 = bucket.\
            new(self.randname(), "data1").add_index(
                "field1_bin", "val1",
            ).add_index("field2_int", int_sign * 1001).store()
        o2 = bucket.\
            new(self.randname(), "data1").add_index(
                "field1_bin", "val2",
            ).add_index("field2_int", int_sign * 1002).store()
        o3 = bucket.\
            new(self.randname(), "data1").add_index(
                "field1_bin", "val3",
            ).add_index("field2_int", int_sign * 1003).store()
        o4 = bucket.\
            new(self.randname(), "data1").add_index(
                "field1_bin", "val4",
            ).add_index("field2_int", int_sign * 1004).store()
